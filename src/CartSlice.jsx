  ...
  reducers: {
    addItem: (state, action) => {
      ...
      if (existingItem) {
        ...
        // Ensure cost is stored as a string as it comes from the product list
        state.items.push({ name, image, cost, quantity: 1 });
      }
    },
    removeItem: (state, action) => {
      // This reducer removes an item from the cart based on its name
      // action.payload is expected to be the name (string) of the item to remove
...
// addItem() for ProductList.jsx
// removeItem() and updateQuantity() for CartItem.jsx
